...
% Revision 4, July 1, 2007, J. Jaeger: 
...
esn.RLS_lambda = 1 ; 
%模块化esn中，增加属性
esn.nSubNet = 1;                 %表示子网络个数
esn.IDsubNet = 1;                %表示现在使用子网络的标识号
esn.trained = 0;
...
for i=1:2:nargs
  switch args{i},
   ...
   case 'nSubNet', esn.subNetNum=args{i+1};esn.trained=zeros(1,args{i+1});
   case 'IDsubNet',esn.IDsubNet=args{i+1};
...
for iSubNet=1:esn.nSubNet
    esn.outputWeights{iSubNet} = zeros(nOutputUnits, nInternalUnits + nInputUnits);
end
